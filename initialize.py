...
if __name__ == "__main__":
    STOCKFISH_PATH = "/opt/homebrew/bin/stockfish"  # Adjust the path to your Stockfish binary
    with chess.engine.SimpleEngine.popen_uci(STOCKFISH_PATH) as engine:
        final_board = random_game(engine, max_moves=30)
        print("Game over:", final_board.is_game_over())
        print("Result:", final_board.result())
        # Print the game moves in order using a fresh board
        print(final_board.fen())
        print("Full move number:", final_board.fullmove_number)
